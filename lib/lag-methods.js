...
Lag = {
  ...
  /**
   * Whether or not to unblock methodName.
   * @param {String} methodName the method name
   * @returns {Boolean} Whether the method shoul be unblocked
   */
  shouldUnblock: function(methodName) {
    return this.getConfigOption('unblock') && !this.isMethodBlocking(methodName);
  },

  /**
   * Wheter or not the given method is configured not to use `unblock`.
   * @param {String} name the method name
   * @returns {boolean} true if we should not call unblock() before delaying the method.
   */
  isMethodBlocking: function(name) {
    var method = lagCollection.findOne({type: 'method', name: name});

    if (!method) {
      return false;
    }
    return !!(method.isBlocking);
  },
  ...
  setForceBlocking: function (name, forceBlocking) {
    lagCollection.upsert({type: 'method', name: name}, {$set: {isBlocking: forceBlocking}});
  },
  ...
  applySettings: function (config) {
    ...
    if (config.hasOwnProperty('forceBlocking')) {
      //unset all
      lagCollection.update({type: 'method'}, {$unset: {isBlocking: 1}}, {multi: true});
      //and set only the relevant ones
      _.each(config.forceBlocking, function (methodName) {
        self.setForceBlocking(methodName, true);
      });
    }
  },
